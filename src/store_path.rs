use crate::hash::{compress_hash, hash_string, HashAlgorithm};
use crate::nix32::Nix32;
...
pub fn check_name(name: &str) -> Result<(), String> {
    ...
    if name.starts_with('.')
        && (name.len() == 1 || name == ".." || name.starts_with(".-") || name.starts_with("..-"))
    {
...
impl StoreDir {
    ...
    /// 5. Return `<nix32>-<name>`
    #[must_use]
    pub fn make_store_path(&self, ty: &str, hash_str: &str, name: &str) -> String {
        let input = format!(
    ...
    #[must_use]
    pub fn make_store_path_from_hash(&self, ty: &str, hash: &[u8], name: &str) -> String {
    ...
    #[must_use]
    pub fn make_output_path(&self, output_name: &str, drv_hash: &[u8], drv_name: &str) -> String {
...
mod tests {
    ...
    fn different_store_dirs_different_paths() {
        let a = StoreDir::new("/nix/store").make_store_path("output:out", "sha256:abc", "foo");
        let b = StoreDir::new("/gnu/store").make_store_path("output:out", "sha256:abc", "foo");
        assert_ne!(a, b);
    }
    ...
    fn parse_valid_store_path() {
        ...
        let (hp, name) = StoreDir::parse_store_path(path).expect("valid store path");
        assert_eq!(hp.len(), 32);
    ...
    fn parse_rejects_bad_hash() {
        assert!(StoreDir::parse_store_path("eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee-x").is_none());
    ...
    fn known_derivation_path_produces_valid_output() {
        ...
        let content_hash = crate::hash::hash_string(HashAlgorithm::Sha256, drv_aterm);
        let suffix = "test.drv";
